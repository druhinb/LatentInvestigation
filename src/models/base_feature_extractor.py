...
class BaseFeatureExtractor(nn.Module):
    ...
    def _setup_hooks(self):
        ...
        if hasattr(self.model, 'encoder') and hasattr(self.model.encoder, 'layer'):
            ...
            logger.warning(f'No transformer blocks found for hooks in {self.model_name}')
    ...
    def get_feature_dim(
        ...
        if isinstance(size_dict, dict) and 'height' in size_dict:
            h = size_dict['height']; w = size_dict['width']
        ...
        feats = self.extract_features(dummy, layers=layers or [-1], feature_type=feature_type)
