...
def compute_classification_metrics(
    ...
    """
    preds_np = predictions.detach().cpu().numpy() if isinstance(predictions, torch.Tensor) else predictions
    targs_np = targets.detach().cpu().numpy() if isinstance(targets, torch.Tensor) else targets
    ...
    metrics.update({'f1_macro': f1_macro, 'f1_micro': f1_micro, 'f1_weighted': f1_weighted})

    # Per-class F1
    if num_classes is None:
        num_classes = max(np.max(targs_np), np.max(pred_classes)) + 1
    f1_per = f1_score(targs_np, pred_classes, average=None, labels=list(range(num_classes)), zero_division=0)
    ...
    if preds_np.ndim == 2 and preds_np.shape[1] > 1:
        for k in [3,5]:
            if k < preds_np.shape[1]:
                ...
                topk_acc = np.mean([targs_np[i] in topk[i] for i in range(len(targs_np))])
