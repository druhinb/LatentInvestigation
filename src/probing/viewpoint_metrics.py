...
<<<<<<< HEAD
    az_p = (
        azimuth_pred.detach().cpu().numpy()
        if isinstance(azimuth_pred, torch.Tensor)
        else np.array(azimuth_pred)
    )
    el_p = (
        elevation_pred.detach().cpu().numpy()
        if isinstance(elevation_pred, torch.Tensor)
        else np.array(elevation_pred)
    )
    az_t = (
        azimuth_target.detach().cpu().numpy()
        if isinstance(azimuth_target, torch.Tensor)
        else np.array(azimuth_target)
    )
    el_t = (
        elevation_target.detach().cpu().numpy()
        if isinstance(elevation_target, torch.Tensor)
        else np.array(elevation_target)
    )
=======
...
<<<<<<< HEAD
    ...
    if categories is not None:
        ...
        for cat in unique:
            ...
            name = (
                category_names[cat]
                if category_names and cat < len(category_names)
                else f"category_{cat}"
            )
