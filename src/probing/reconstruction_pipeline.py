...
class ReconstructionDataset(Dataset):
    ...
    def __init__(self, view_data: torch.Tensor, voxels: torch.Tensor, ids: list, cats: list):
...
class ReconstructionPipeline(BasePipeline):
    ...
    def _process_batch(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, Dict]:
        ...
        feats_dict = self.image_pipeline.extract_features(
            flat_imgs, layers=getattr(self, 'layers', None), feature_type=getattr(self, 'feature_type', 'cls_token')
        )
        feats_flat = list(feats_dict.values())[0] if feats_dict else torch.empty(B*NV, 0)
        ...
        # metadata
        meta = {'model_ids': list(batch['model_id']), 'categories': list(batch['category'])}
    ...
    def create_dataset(
        ...
        feats, voxels, meta = self.extract(dataloader, cache_key=cache_key, force_recompute=force_recompute)
        return ReconstructionDataset(feats, voxels, meta.get('model_ids', []), meta.get('categories', []))
